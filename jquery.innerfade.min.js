(function($){$.fn.innerFade=function(options){return this.each(function(){$.innerFade(this,options);});};$.innerFade=function(container,options){var settings={'animationtype':'fade','easing':'linear','speed':'normal','type':'sequence','timeout':2000,'containerheight':'auto','runningclass':'innerFade','children':null,'cancelLink':'.cancel','pauseLink':'.pause','prevLink':'.prev','nextLink':'.next','indexContainer':null};if(options){$.extend(settings,options);}
...
var current='';var last='';if(settings.type=="random"){last=Math.floor(Math.random()*elements.length);do{current=Math.floor(Math.random()*elements.length);}while(last==current);$.innerFade.next(container,elements,settings,current,last);$(elements[last]).show();}else if(settings.type=='random_start'){settings.type='sequence';current=Math.floor(Math.random()*(elements.length));$.innerFade.next(container,elements,settings,(current+1)%elements.length,current);$(elements[current]).show();}else{$.innerFade.next(container,elements,settings,0,elements.length-1);$(elements[0]).show();}
